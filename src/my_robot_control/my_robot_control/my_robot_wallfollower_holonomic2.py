...
class WallFollower(Node):
    def __init__(self):
        ...
        # -------------------------------
        # WALL LOST MODE STATE
        # -------------------------------
        self.wall_lost = False
        self.wall_lost_start = None
        self.wall_lost_duration = 0.7   # temps per buscar la paret
    ...
    def laser_callback(self, scan):
        ...
        # ==========================================================
        # >>>>>>>>>>>>>>>>>>>>    WALL LOST MODE (after other rules)    <<<<<<<<<<<<<<<<<<<<<<
        # ==========================================================
        if not math.isfinite(min_right) and not math.isfinite(min_fr_right):
            now = self.get_clock().now().nanoseconds * 1e-9
            if not self.wall_lost:
                self.wall_lost = True
                self.wall_lost_start = now
                self.get_logger().info("⚠️ WALL LOST → searching right")

            if now - self.wall_lost_start < self.wall_lost_duration:
                twist.linear.x = self.v_lin * 0.4
                twist.linear.y = -self.v_lin * 0.6
                twist.angular.z = 0.0
                action = "WALL LOST → STRAFE RIGHT to recover"
            else:
                self.wall_lost = False
        else:
            self.wall_lost = False
